...
#THIS IS DT
function buildControllerNormalized(ic,capacity,N,Q_OCV,q_OCV,Q_POCV,q_POCV,Q_NOP,q_NOP,Q_POP,q_POP,Q_OOP,q_OOP,h,c,τ_ohm;max_T=333,pl_tol=0,top_SOC=0.9,chatter_weight=1,time_weight=1)
    #Construct IPOPT controller
    model = Model(Ipopt.Optimizer)
    set_optimizer_attribute(model, "max_iter", 1000000)
    #Build model matrices
    ...
    for i=1:N
        ...
        @NLconstraint(model,(OOP[i+1]-OOP[i])==DOOP[i]*DT)
        @constraint(model,DOOP[i]==(-OOP[i]+x[i,:]'*Q_OOP*x[i,:]+q_OOP'*x[i,:])/τ_ohm)
        @NLconstraint(model,(z[i+1]-z[i])==(-I[i]/3600)*DT)
        @NLconstraint(model,(T[i+1]-T[i])==(-h*(T[i]-T_amb)-I[i]*oneC*(V[i]-OCV[i]))*DT/(c))
        #Trivial Constraints (definitions)
        @constraint(model,x[i,3]==I[i])
        @constraint(model,x[i,2]==z[i])
        ##CONSTRAINTS ON OPERATION
